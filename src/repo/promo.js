...
const getPromo = (queryparams) => {
  return new Promise((resolve, reject) => {
    let query = `select id , cuponcode, discount from promos`;
...
const createPromo = (body) => {
  return new Promise((resolve, reject) => {
    const query = "insert into promos (cuponcode, discount) values ($1,$2) ";
    const { cuponcode, discount } = body;
    postgreDb
      .query(query, [cuponcode, discount])
      .then(() => {
