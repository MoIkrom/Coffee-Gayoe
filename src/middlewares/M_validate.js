module.exports = {
  body: (...allowedKeys) => {
    return (req, res, next) => {
      ...
      req.body = newBody;
      next();
      if (Object.keys(newBody).length === 0) return res.status(400).json({ msg: "Please Insert Requirement" });

      if (Object.keys(newBody).length !== allowedKeys.length) return res.status(400).json({ msg: " Input key does not match " });
    };
  },
  params: (...allowedKeys) => {
    return (req, res, next) => {
...
// validate.promoData = (req, res, next) => {
//   const { body } = req;
...
//   const { product_name, category, sort, order } = req.query;
//   if (product_name) {
...
//   }
//   if (category) {
//     if (category !== "food" && category !== "coffee" && category !== "non coffee") {
//       return res.status(400).json({
//         error: "Invalid input category !",
//       });
//     }
//   }
//   if (sort) {
